...
#include <proc/pic18f26k40.h> //Possibly not needed, depends on XC8 compiler install
...
void PortsConfig(void)
{
    ...
    RC2PPS = 0x12; //Set RC2 as MSSP2 SDA
    RC3PPS = 0x11; //Set RC3 as MSSP2 SCL
    SSP2DATPPS = 0b00010010; //SSP2DAT input set as RC2
    SSP2CLKPPS = 0b00010011; //SSP2CLK input set as RC3
...
void SSP1Config (void)
{
    ...
    SSP1CON3bits.PCIE = 0; //Disable interrupts on stop condition detect
...
void SSP2Config (void)
{
    ...
    SSP2ADD = 39; //Set BRG divider for 400kHz baud. | 100kHz = 159, 400kHz = 39
    SSP2CON1bits.SSPM = 0b1000; //I2C Master Mode
    SSP2STATbits.SMP = 1; //Slew Rate set for 400kHz
...
void HandleTMR4(void)
{
    ...
    {
        SSP1Config(); //Setup SSP1 now that the program has been delayed
        LATA = SSP1ADD; //Show I2C address on LATA until it is over written in RequestSensorData()
        T4CONbits.T4ON = 0; //Turn off TMR4
    }
    pFlag &= 0b01111111; //Clear TMR4 pFlag to only run handle once
}
...
{    
    for(;;)
    {
        if(I2C2Scan() == 0x00) {LATA = 0xEE; break;} //Scan to find new sensor
        if(SaveSensor() == 0x00) {LATA = 0xFF; break;} else {LATA = sensorEEADR;} //Save new sensor
        break;
    }
}
...
void RequestSensorData (void)
{
    ...
    for(i = 0x10; i <= lastSensor; i+=2)
    {
        ...
        if(sensorID == 'L') {if(distData[1] != 0) {distData[0] = 0xFF;}} //Cap the LUNA's distance to 0xFF

        for(i2 = 0; i2 <= (distanceBytes - 1); i2++) //Export distance data to slave registers
        {                                            // ////
            byteCount++;                             // ///
            I2C1Registers[byteCount] = distData[i2]; // //
        }                                            // /
    }
    I2C1Registers[0] = byteCount; //Save number of sensors found
    LATA = I2C1Registers[0]; //Show number of sensors found
}
...
{   
    ...
    else
    {
        ...
        //SSP1Config(); //SSP1 will be configured by TMR4
        TMR4Config();
        SSP2Config();
    }
...
void TMR4_ISR(void)
{
    ...
    PIR4bits.TMR4IF = 0; //Clear TMR4 Flag
}
...
void I2C1_ISR(void)
{
    ...
    switch (I2C1Status)
    {
        ...
        case 1: //Currently in a master write to slave conversation
            ...
            I2C1RxData = SSP1BUF;
            I2C1RegIndex = I2C1RxData; //Read buffer to assign register
            SSP1CON1bits.CKP = 1; //Release clock
            I2C1Status = 0;
        ...
        case 2: //Currently in a master read from slave conversation
            //Check CKP to see if a NACK was detected
